...
function do_llvm() {
    ...
    "$base"/build-llvm.py \
        ...
        --multicall \
        -D CMAKE_INSTALL_DO_STRIP=ON LLVM_ENABLE_ZSTD=ON LLVM_ENABLE_PLUGINS=OFF LLVM_VENDOR="Olzhas0986" LLVM_INCLUDE_TESTS=OFF LLVM_INCLUDE_EXAMPLES=OFF LLVM_INCLUDE_BENCHMARKS=OFF \
        "${extra_args[@]}"
}
